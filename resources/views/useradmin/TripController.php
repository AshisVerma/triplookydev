 ...
 $day= round($day/86400);
    ...
    public function tripSummary(Request $request){
      ...
      foreach(session()->get('city') as $city_list){
       ...
       $accomodation[$key]=Accomodation::select("id","image","name","price","link","city")->where("type",$accomodation_list)->get()->toArray();
       if(!empty($accomodation[$key])){
       ...
       $transportation[$key]=Transportation::select("id","image","name","price","link","city",'type')->where("type",$transportation_list)->where("status",1)->get()->toArray();
      ...
      $activity[$key]=TourAndActivity::select("id","image","name","price","link","city")->where("type",$activity_list)->where("city",$city_list)->get()->toArray();
      }
       }
       ...
       foreach(session()->get('city') as $city_list){
        ...
        $fooddrink[$key]=FoodDrink::select("id","image","name","price","link","city")->where("type",$fooddrink_list)->where("city",$city_list)->where("status",1)->get()->toArray();
      }
       }
     ...
     $days   = floor(($dateDifference - $years * 365 * 60 * 60 * 24 - $months * 30 * 60 * 60 *24) / (60 * 60 * 24));
        ...
        $city[$key]=City::select("id","city_name","image")->where("id",$city_list)->get()->toArray();
         ...
         for($i=0;$i<count($city);$i++){
          $data=base64_decode($city[$i]['day']);
          $id=base64_decode($city[$i]['data']);
          for($j=1;$j<=$data;$j++){
            $city_details_find=City::select("city_name","image","id")->where("id",$id)->get()->toArray();
