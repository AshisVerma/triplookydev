      ...
      <div class="container-fluid bodyPad">
        ...
        <h4 style="color: #fa842d;"><b class="ct">{{$city[$i][$j]['city_name']}}</b></h4>
                        ...
                        @if(count($city)-1==$i)
                         ...
                         <div class="carousel-item" data-bs-interval="false" data="{{$city[count($city)-1][0]['id']}}">
                              <div class="carousel-caption d-none d-md-block px-0">
        <h4 style="color: #fa842d;"><b class="ct">{{$city[count($city)-1][0]['city_name']}}</b></h4>
        
      </div> 
                           <img src="{{url('')}}/city-image/images/{{$city[count($city)-1][0]['image']}}" class="imgM">
               ...
               <div class="swiper-container transportation_sl" id="transportation_slider">
                 ...
                 <div class="swiper-slide selct">
                     ...
                     <p class="priceTag"><span>Starting from</span> <span class='transportation-price-data'>
                        ...
                        <price>{{(int)(session('change_price')*$transportation[$i]['price'])}}</price>
                     </p>
                      <a href="javascript:void(0);" class="info-cities transport-info"  data="{{$transportation[$i]['id']}}" type=""><i class="fa-solid fa-circle-info"></i> Details</a>
                     <img src="{{url('')}}/transportation-image/images/{{$transportation[$i]['image']}}">                    
                     <h3 class="text-center citiesName"></h3>
                     <a href="{{$transportation[$i]['link']}}" class="bookBtn" target="_blank">Book</a>
                     <a href="javascript:void(0)" class="addTotrip add-transportation-btn" data="{{$transportation[$i]['id']}}" onclick="transportation(this)">Add to your trip</a>
                   </div>
                  </div>
                   @endfor
                  <!---Transportation  list display selected from Transportation page accoding to city  ---->
            ...
            <!-- Close -->
            <div class="text-center sticy_pos">
               <a href="/transport" class="btn btn-default">Previous</a>
...
//city next move and day next move and filteration on accomodation,tour and activity,fooddrink,transportation
 ...
 //end filter function call //
  ...
  function accomodation(link){
    ...
    //check last day not book accomodation
    var all_slider=$(".carousel-item");
    var index=$(".carousel-item.active").index();
    if(all_slider.length-1==index){
       swal({
                 title: "You can`t book accomodation for this date",
    ...
    //end last day not book accomodation
  ...
  $(document).ready(function(){
   ...
   console.log(response);
           $(".accomodation-carousel-inner-box").html(`<div class="carousel-item active" style="background-image: url({{url('')}}/accomodation-image/images/`+response.data.image+`"></div>`);
            ...
            $(".accomodation-rating-price").html("{{session('currency_sign')}}"+({{session('change_price')}}*response.data.price));
   ...
   console.log(response);
           $(".fooddrink-carousel-inner-box").html(`<div class="carousel-item active" style="background-image: url({{url('')}}/food-drink-image/images/`+response.data.image+`"></div>`);
            ...
            $(".fooddrink-rating-price").html("{{session('currency_sign')}}"+({{session('change_price')}}*response.data.price));
            $(".fooddrink-star").html("");
            for(i=0;i<Number(response.data.star);i++){
             var star=document.createElement("I");
             star.className="fa fa-star text-warning";
...
// -------------- END RESET PRESELECTED DATA------------//
 ...
 //city carousl slide coding start here
   ...
   $(".day-prev-btn").click(function(){
    ...
    $(document).on('click', '.day-next-btn',function(){
       let city_id=null;
          city_id=$("#city-carousel .carousel-item.active").attr("data");
...
//filter all data 
    ...
    $(".accomodation-div").each(function(){
      if($(this).attr("data")==id){
         console.warn("id => ",$(this).attr("data"))
