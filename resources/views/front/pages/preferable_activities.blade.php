        ...
        <div class="d-flex align-items-center wrap">
         ...
         @if(session()->has('activity'))
          @if(in_array($data[$i]['name'],array_values(session('activity'))))
           <div class="activity-select-box w-20 wow fadeInLeftBig" data-wow-duration="2s" data-wow-delay="0.2s" data="{{$data[$i]['name']}}">
...
var activity_data=[];
  var activity=[];
  $(".activity-select-box").each(function(i,data){
  $(this).click(function(){
    if(activity.indexOf($(this).attr("data"))==-1){
      
      activity.push($(this).attr("data"));
    }
  ...
  activity_data=activity;
    ...
    var data=JSON.parse(sessionStorage.getItem("tour"));
     if(activity.length>0){
       for(activity of activity){
   if(data.activity.indexOf(activity)==-1){
     data.activity.push(btoa(activity));
   }
